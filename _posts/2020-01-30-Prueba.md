...
MAE (mean absolute error):$\frac{\sum_{i=1}^{n}|y − \hat{y}|}{n}$, sumatorio del valor absoluto de cada uno de los errores dividido por el número de pruebas (n).

RMSE (root mean square error):$\sqrt{\frac{\sum_{i=1}^{n}(y-\hat{y})^2}{n}}$, sumatorio del cuadrado de los errores dividido por el numero de ejemplos que tenemos (n). Finalmente obtiene la raiz cuadrada del número calculado.
