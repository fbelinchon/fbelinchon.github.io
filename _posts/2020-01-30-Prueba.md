...
**Machine Learning** es un campo dentro de la inteligencia artificial que se centra en modelos de aprendizaje. Estos modelos aprenden de los ejemplo que se les proporciona para detectar patrones y obtener predicciones.
...
Las **redes neuronales** pertenecen al ámbito de Machine Learning. Hablamos de modelos que son capaces de aprender sin ser explicitamente programados. Al trabajo con redes neuronales también se le conoce como Deep Learning porque los modelos pueden tener multiples capas con las que conseguimos una deteccción de patrones mucho más precisa. Gracias a esta capacidad las redes neuronales han conseguido resolver problemas que hace unos años parecían inalcanzable, como por ejemplo.
...
$error=(y − \hat{y})$ la diferencia entre el valor pedicho por nuestro modelo y el valor real.

Esta es precisamente la definición de la función de error "loss function". Bueno, normalmente se toma el valor absoluto o la diferencia al cuadrado. Tenemos que calcular el error medio cometido en todos nuestros ejemplos.
